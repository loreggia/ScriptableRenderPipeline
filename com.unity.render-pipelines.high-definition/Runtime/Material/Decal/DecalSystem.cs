...
namespace UnityEngine.Experimental.Rendering.HDPipeline
{
    public class DecalSystem
    {
        ...
        {
            if (debugDisplaySettings.m_Data.decalsDebugSettings.displayAtlas)
            {
                using (new ProfilingSample(cmd, "Display Decal Atlas", CustomSamplerId.DisplayDebugDecalsAtlas.GetSampler()))
                {
                    cmd.SetViewport(new Rect(x, y, overlaySize, overlaySize));
                    HDUtils.BlitQuad(cmd, Atlas.AtlasTexture, new Vector4(1, 1, 0 , 0), new Vector4(1, 1, 0, 0), (int)debugDisplaySettings.m_Data.decalsDebugSettings.mipLevel, true);
