...
// If we are in the editor, we have to take the current target build platform as the graphic device type is always the same
        ...
        protected override void Render(ScriptableRenderContext renderContext, Camera[] cameras)
        {
            ...
            var renderRequests = new List<ScheduleRenderState>();
            ...
            foreach (var camera in cameras)
            {
                ...
                var hdCamera = HDCamera.Get(camera) ?? HDCamera.Create(camera);
                ...
                using (new ProfilingSample(cmd, "HDRenderPipeline::Render", CustomSamplerId.HDRenderPipelineRender.GetSampler()))
                {
                    ...
                    hdCamera.UpdateStereoDependentState(cullingParams);
                    ...
                    if (hdCamera.frameSettings.enableDecals)
                    {
                        using (new ProfilingSample(cmd, "DBufferPrepareDrawData", CustomSamplerId.DBufferPrepareDrawData.GetSampler()))
                        {
                            ...
                            // TODO: to move in ScheduleRender
                    ...
                    renderRequests.Add(new ScheduleRenderState
                    {
                        camera = camera,
                        hdCamera = hdCamera,
                    ...
                    // TODO: Add probe to render this frame in renderRequests
                }
                ...
                for (int i = 0; i < renderRequests.Count; ++i)
                {
                    ...
                    ScheduleRender(
                        ...
                        request.camera,
                        request.hdCamera,
                        request.frameSettings,
                        request.postProcessLayer,
                        request.cullingResults,
                        request.scriptableCullingParameters
                    );
                }
        ...
        {
            hdCamera.Update(currentFrameSettings, postProcessLayer, m_VolumetricLightingSystem, m_MSAASamples);
            hdCamera.UpdateStereoDependentState(scriptableCullingParameters);
            ...
            if (hdCamera.frameSettings.enableDecals)
            {
                using (new ProfilingSample(cmd, "DBufferPrepareDrawData", CustomSamplerId.DBufferPrepareDrawData.GetSampler()))
                {
                    // TODO: feed with proper culling data
                    //DecalSystem.instance.EndCull();
                    m_DbufferManager.enableDecals = true;              // mesh decals are renderers managed by c++ runtime and we have no way to query if any are visible, so set to true
                    DecalSystem.instance.UpdateCachedMaterialData();    // textures, alpha or fade distances could've changed
                    DecalSystem.instance.CreateDrawData();              // prepare data is separate from draw
                    DecalSystem.instance.UpdateTextureAtlas(cmd);       // as this is only used for transparent pass, would've been nice not to have to do this if no transparent renderers are visible, needs to happen after CreateDrawData
                    throw new NotImplementedException("TODO: provide decal culling data for this rendering");
                }
            }
