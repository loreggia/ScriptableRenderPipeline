...
namespace UnityEngine.Experimental.Rendering.HDPipeline
{
    public class HDRenderPipeline : UnityEngine.Rendering.RenderPipeline
    {
        ...
        public HDRenderPipeline(HDRenderPipelineAsset asset)
        {
            ...
            m_DebugDisplaySettings.m_Data.msaaSamples = m_MSAASamples;
...
// If we are in the editor, we have to take the current target build platform as the graphic device type is always the same
            ...
            foreach (var camera in cameras)
            {
                ...
                using (new ProfilingSample(cmd, "HDRenderPipeline::Render", CustomSamplerId.HDRenderPipelineRender.GetSampler()))
                {
                    ...
                    else
                    {
                        ...
                        m_MSAASamples = m_DebugDisplaySettings.m_Data.msaaSamples;
                    ...
                    if (debugDisplaySettings.m_Data.lightingDebugSettings.debugLightingMode == DebugLightingMode.LuxMeter)
                    {
                    ...
                    else
                    {
                        ...
                        if (hdCamera.frameSettings.enableContactShadows && m_CurrentDebugDisplaySettings.m_Data.fullScreenDebugMode == FullScreenDebugMode.ContactShadows)
                        {
        ...
        void RenderDebugViewMaterial(CullingResults cull, HDCamera hdCamera, ScriptableRenderContext renderContext, CommandBuffer cmd)
        {
            ...
            {
                if (m_CurrentDebugDisplaySettings.m_Data.materialDebugSettings.IsDebugGBufferEnabled() && hdCamera.frameSettings.shaderLitMode == LitShaderMode.Deferred)
                {
        ...
        public void ApplyDebugDisplaySettings(HDCamera hdCamera, CommandBuffer cmd)
        {
            ...
            if (m_CurrentDebugDisplaySettings.IsDebugDisplayEnabled() ||
                m_CurrentDebugDisplaySettings.m_Data.colorPickerDebugSettings.colorPickerMode != ColorPickerDebugMode.None)
            {
                // This is for texture streaming
                m_CurrentDebugDisplaySettings.UpdateMaterials();

                var lightingDebugSettings = m_CurrentDebugDisplaySettings.m_Data.lightingDebugSettings;
        ...
        {
            if (m_CurrentDebugDisplaySettings.m_Data.colorPickerDebugSettings.colorPickerMode != ColorPickerDebugMode.None || m_DebugDisplaySettings.m_Data.falseColorDebugSettings.falseColor || m_DebugDisplaySettings.m_Data.lightingDebugSettings.debugLightingMode == DebugLightingMode.LuminanceMeter)
            {
        ...
        {
            bool fullScreenDebugEnabled = m_CurrentDebugDisplaySettings.m_Data.fullScreenDebugMode != FullScreenDebugMode.None;
            bool lightingDebugEnabled = m_CurrentDebugDisplaySettings.m_Data.lightingDebugSettings.shadowDebugMode == ShadowMapDebugMode.SingleShadow;
        ...
        {
            if (debugMode == m_CurrentDebugDisplaySettings.m_Data.fullScreenDebugMode)
            {
        ...
        void PushFullScreenDebugTextureMip(HDCamera hdCamera, CommandBuffer cmd, RTHandleSystem.RTHandle texture, int lodCount, Vector4 scaleBias, FullScreenDebugMode debugMode)
        {
            if (debugMode == m_CurrentDebugDisplaySettings.m_Data.fullScreenDebugMode)
            {
                var mipIndex = Mathf.FloorToInt(m_CurrentDebugDisplaySettings.m_Data.fullscreenDebugMip * (lodCount));
        ...
        void RenderDebug(HDCamera hdCamera, CommandBuffer cmd, CullingResults cullResults)
        {
            ...
            using (new ProfilingSample(cmd, "Render Debug", CustomSamplerId.RenderDebug.GetSampler()))
            {
                ...
                if (NeedsFullScreenDebugMode() && m_FullScreenDebugPushed)
                {
                    ...
                    m_DebugFullScreen.SetFloat(HDShaderIDs._FullScreenDebugMode, (float)m_CurrentDebugDisplaySettings.m_Data.fullScreenDebugMode);
                    HDUtils.PackedMipChainInfo info = m_SharedRTManager.GetDepthBufferMipChainInfo();
                    m_DebugFullScreen.SetInt(HDShaderIDs._DebugDepthPyramidMip, (int)(m_CurrentDebugDisplaySettings.m_Data.fullscreenDebugMip * info.mipLevelCount));
                ...
                // Then overlays
                float x = 0;
                float overlayRatio = m_CurrentDebugDisplaySettings.m_Data.debugOverlayRatio;
                ...
                var lightingDebug = m_CurrentDebugDisplaySettings.m_Data.lightingDebugSettings;
                ...
                if (m_CurrentDebugDisplaySettings.m_Data.colorPickerDebugSettings.colorPickerMode != ColorPickerDebugMode.None || m_CurrentDebugDisplaySettings.m_Data.falseColorDebugSettings.falseColor || m_CurrentDebugDisplaySettings.m_Data.lightingDebugSettings.debugLightingMode == DebugLightingMode.LuminanceMeter)
                {
                    ColorPickerDebugSettings colorPickerDebugSettings = m_CurrentDebugDisplaySettings.m_Data.colorPickerDebugSettings;
                    FalseColorDebugSettings falseColorDebugSettings = m_CurrentDebugDisplaySettings.m_Data.falseColorDebugSettings;
            ...
            using (new ProfilingSample(cmd, "ClearBuffers", CustomSamplerId.ClearBuffers.GetSampler()))
            {
                ...
                using (new ProfilingSample(cmd, "Clear HDR target", CustomSamplerId.ClearHDRTarget.GetSampler()))
                {
                    if (hdCamera.clearColorMode == HDAdditionalCameraData.ClearColorMode.BackgroundColor ||
                        ...
                        m_DebugDisplaySettings.m_Data.lightingDebugSettings.debugLightingMode == DebugLightingMode.LuxMeter ||
                        ...
                        )
                    {
                        ...
                        if (m_DebugDisplaySettings.m_Data.lightingDebugSettings.debugLightingMode == DebugLightingMode.LuxMeter)
